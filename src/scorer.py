    ...
    if "-" in frag:
        ...
        for ln_frag in lname_frags:
            if ln_frag in LASTNAMES:
                found_count += 1
        if found_count == len(lname_frags):
            return True
...
def isoccupation(frag):
    ...
    if frag in OCCUPATIONS:
        return True
    if frag in WORKPLACES:
        return True
    ...
    if len(frag) > MIN_OCCUPATION_LAZY_MATCH_LENGH:
        matches = get_close_matches(
            frag, WORKPLACES, n=1, cutoff=MIN_OCCUPATION_LAZY_MATCH_THRESHOLD
        )
        if len(matches) > 0:
            if DEBUG_TO_STDOUT:
...
def joiner(matched_fragments, num_identified):
    ...
    if len(matched_fragments["unknown"]) <= ACCEPT_MAX_UNKNOWN_FRAGS:
        ...
        for pos, (title, spos) in enumerate(matched_fragments["unknown"].items()):
            ...
            if max_unknowns_consumed >= ACCEPT_MAX_UNKNOWN_FRAGS:
                break
    ...
    for pos, (street, spos) in enumerate(matched_fragments["streets"].items()):
        ...
        score = statistics.mean([lname_score, fname_score, occ_score, street_score])
...
def inspect():
        ...
        for line in open(vol, "r"):
            ...
            for offset, frag in enumerate(line_fragments):
                ...
                if isstreet(frag) and not (
                    frag in scored_line["occupations"]
                    and len(scored_line["occupations"]) < 1
                ):
            ...
            if (
                len(scored_line["lastnames"]) > 0
                and len(scored_line["givennames"]) > 0
                and len(scored_line["streets"]) > 0
                and len(scored_line["occupations"]) > 0
            ):
                known_good += 1
            else:
                known_bad += 1
            ...
            if len(scored_line["lastnames"]) > 0:
                ...
                for lname in scored_line["lastnames"]:
                    ...
                    if (
                        gnisln(lname, scored_line["givennames"])
                        and len(scored_line["givennames"]) == 1
                        and len(scored_line["lastnames"]) == 1
                    ):
            ...
            if total_fragments == total_found or (
                len(scored_line["unknown"]) > 0 and ACCEPT_MAX_UNKNOWN_FRAGS > 0
            ):
                ...
                match_ratio = SequenceMatcher(
                    lambda x: x in ",", line.rstrip(), mint_line
                ).ratio()
                ...
                if match_ratio < SEQUENCE_MATCH_THRESHOLD or score == 0:
                    if DEBUG_TO_STDOUT:
                        ...
                        print(
                            "similarity too low: %s"
                            % (match_ratio, SEQUENCE_MATCH_THRESHOLD)
                        )
                ...
                if DEBUG_TO_STDOUT:
                    ...
                    print("matched segments: %s\n\n" % scored_line)
                    print(
                        "total fragmentss vs total found: %s:%s"
                        % (total_fragments, total_found)
                    )
    ...
    print(
        "Results: %s (total), %s (good), %s (bad), %s (seq ok)"
        % (total_lines, known_good, known_bad, seq_ok)
    )
