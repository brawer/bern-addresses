...
STREET_ABBREVS_PATH = os.path.join(
    os.path.dirname(__file__), "..", "street_abbrevs.csv"
)
STREET_ABBREVS = {
...
def fix_conjunctions():
    for vol in list_volumes():
        ...
        with open(vol + ".tmp", "w") as out:
            ...
            for line in open(vol, "r"):
                ...
                if line_stash != "":
                    ...
                    line_stash_txt, line_stash_pos = [
                        x.strip() for x in line_stash.split("#")
                    ]
                    line_stash_pos_all = line_stash_pos.split(";")
                    line_stash_x, line_stash_y, _w, _h = [
                        int(x) for x in line_stash_pos_all[0].split(",")
                    ]
                    ...
                    line_x, line_y, _w, _h = [
                        int(x) for x in line_pos_all[0].split(",")
                    ]
                    ...
                    if abs(line_stash_x - line_x) < GLUE_AT_X_OFFSET or (
                        abs(line_stash_y - line_y) < GLUE_AT_Y_OFFSET
                        and abs(line_stash_x - line_x) < GLUE_AT_Y_IF_X_LESS_THAN
                    ):
                        # mint new line
            ...
            for line in open(vol, "r"):
                ...
                if line_stash != "":
                    ...
                    if abs(line_stash_x - line_x) < COMMA_GLUE_AT_X_OFFSET or (
                        abs(line_stash_y - line_y) < COMMA_GLUE_AT_Y_OFFSET
                        and abs(line_stash_x - line_x) < COMMA_GLUE_AT_Y_IF_X_LESS_THAN
                    ):
        ...
        for k, line in enumerate(line_buffer):
            ...
            prev_line_txt, prev_line_pos = [
                x.strip() for x in line_buffer[k - 1].split("#")
            ]
            ...
            if prev_first_line_seg + cur_first_line_seg in OCCUPATIONS:
                minted_line = (
                    f"{prev_line_txt}{cur_line_txt}  # {prev_line_pos};{cur_line_pos}"
                )
            ...
            cur_first_line_seg = cur_line_txt.replace(",", " ").split()[0].strip()
            if (
                prev_first_line_seg + cur_first_line_seg in STREETS
                or prev_first_line_seg + cur_first_line_seg in STREET_ABBREVS
            ):
                minted_line = (
                    f"{prev_line_txt}{cur_line_txt}  # {prev_line_pos};{cur_line_pos}"
                )
            ...
            # a lastname
            if (
                first_line_seg in STREETS
                or first_line_seg in STREET_ABBREVS
                or first_line_seg in known_street_frags
            ) and first_line_seg not in LASTNAMES:
                prev_x, prev_y, _w, _h = [
                    int(x) for x in prev_line_pos.split(";")[0].split(",")
                ]
                ...
                cur_x, cur_y, _w, _h = [
                    int(x) for x in cur_line_pos.split(";")[0].split(",")
                ]
                ...
                if (
                    abs(prev_x - cur_x) < GLUE_STREET_WITHIN
                    or abs(prev_y - cur_y) < GLUE_STREET_WITHIN
                ) and prev_y < cur_y:
                    # trim trailing chars
                    ...
                    sep = (
                        ""
                        if any(line.startswith(x) for x in known_street_frags)
                        else ", "
                    )
        ...
        for k, line in enumerate(line_buffer):
            ...
            if (
                last_line_seg in JOIN_WORDS
                and not first_line_seg == "-"
                and not first_line_seg in LASTNAMES
                and not first_line_seg in GIVENNAMES
            ):
                minted_line = (
                    f"{prev_line_txt} {cur_line_txt}  # {prev_line_pos};{cur_line_pos}"
                )
